...
public class DemandeListeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<DemandeAdministrative> registreDemandes = em.createQuery(
                    "SELECT d FROM DemandeAdministrative d LEFT JOIN FETCH d.citoyenRequerant LEFT JOIN FETCH d.typeActe",
            ...
            // 💡 CORRECTION : Utilisation du dictionnaire dynamique en mémoire
            // (Ultra-rapide)
            List<TypeActe> listeTypes = new ArrayList<>(dao.typeActe.getCache().values());
            List<Citoyen> listeCitoyens = dao.citoyen.findMany();

            // 4. Génération du formulaire via sa Factory dédiée
            // 💡 CORRECTION : La signature est allégée (listePriorites retirée)
    ...
    private void initialiserDonneesSiVide() {
        ...
        try {
            ...
            try {
                tx.begin();
                System.out.println(">>> [DemandeListeServlet] Table vide. Génération du jeu d'essai via JPADao...");

                // 💡 Instanciation du DAO
                JPADao dao = new JPADao(em);

                // 1. Initialisation du catalogue des prestations en base (Nouveau système)
                TypeActe tCelibat = dao.typeActe.saveOrUpdate("CERT_CELIBAT", "Certificat de Célibat", 1500, "",
                        CategorieActe.CERTIFICAT, "Attestation de statut matrimonial");
                TypeActe tConstruire = dao.typeActe.saveOrUpdate("AUTORISATION_CONSTRUIRE",
                        "Autorisation de Construire", 50000, "",
                        CategorieActe.AUTORISATION, "Permis de construction");
                TypeActe tLegalisation = dao.typeActe.saveOrUpdate("LEGALISTION_SIGNATURE", "Légalisation de Signature",
                        1000, "",
                        CategorieActe.LEGALISTION, "Certification matérielle de signature");

                // 2. Initialisation de quelques citoyens témoins
                Citoyen c1 = dao.citoyen.create("Fouda", "Amina", "", "Yaoundé", Sexe.F, LocalDate.of(1990, 1, 1), null,
                        SituationMatrimoniale.CELIBATAIRE, StatutCitoyen.ACTIF);
                Citoyen c2 = dao.citoyen.create("Nguemo", "Patrick", "", "Yaoundé", Sexe.M, LocalDate.of(1990, 1, 1),
                        null, SituationMatrimoniale.CELIBATAIRE, StatutCitoyen.ACTIF);
                Citoyen c3 = dao.citoyen.create("Kamga", "Hubert", "", "Yaoundé", Sexe.M, LocalDate.of(1990, 1, 1),
                        null, SituationMatrimoniale.CELIBATAIRE, StatutCitoyen.ACTIF);
                Citoyen c4 = dao.citoyen.create("Amougou", "Sylvain", "", "Yaoundé", Sexe.M, LocalDate.of(1990, 1, 1),
                        null, SituationMatrimoniale.CELIBATAIRE, StatutCitoyen.ACTIF);
                Citoyen c5 = dao.citoyen.create("Bella", "Chantal", "", "Yaoundé", Sexe.F, LocalDate.of(1990, 1, 1),
                        null, SituationMatrimoniale.CELIBATAIRE, StatutCitoyen.ACTIF);
                Citoyen c6 = dao.citoyen.create("Atangana", "Dieudonné", "", "Yaoundé", Sexe.M,
                        LocalDate.of(1990, 1, 1), null, SituationMatrimoniale.CELIBATAIRE, StatutCitoyen.ACTIF);
                Citoyen c7 = dao.citoyen.create("Mbarga", "Jean-Paul", "", "Yaoundé", Sexe.M, LocalDate.of(1990, 1, 1),
                        null, SituationMatrimoniale.CELIBATAIRE, StatutCitoyen.ACTIF);
                Citoyen c8 = dao.citoyen.create("Mballa", "Jeanne", "", "Yaoundé", Sexe.F, LocalDate.of(1990, 1, 1),
                        null, SituationMatrimoniale.CELIBATAIRE, StatutCitoyen.ACTIF);

                // 3. Insertion du registre des demandes connectées aux citoyens (Utilise
                // maintenant les objets TypeActe)
                dao.demande.create("", tCelibat, c1, LocalDateTime.of(2026, 6, 8, 9, 30), StatutDemande.SOUMISE,
                        PrioriteDemande.NORMALE, null);
                dao.demande.create("", tConstruire, c2, LocalDateTime.of(2026, 6, 8, 14, 15), StatutDemande.SOUMISE,
                        PrioriteDemande.HAUTE, null);
                dao.demande.create("", tLegalisation, c3, LocalDateTime.of(2026, 6, 9, 8, 0), StatutDemande.SOUMISE,
                        PrioriteDemande.URGENTE, null);
                dao.demande.create("", tLegalisation, c4, LocalDateTime.of(2026, 6, 5, 11, 0), StatutDemande.EN_COURS,
                        PrioriteDemande.NORMALE, null);
                dao.demande.create("", tCelibat, c5, LocalDateTime.of(2026, 6, 6, 16, 45), StatutDemande.EN_COURS,
                        PrioriteDemande.HAUTE, null);
                dao.demande.create("", tConstruire, c6, LocalDateTime.of(2026, 5, 12, 10, 20), StatutDemande.VALIDEE,
                        PrioriteDemande.BASSE, "certif_res_0012.pdf");
                dao.demande.create("", tConstruire, c7, LocalDateTime.of(2026, 5, 20, 15, 30), StatutDemande.REJETEE,
                        PrioriteDemande.NORMALE, null);
                dao.demande.create("", tLegalisation, c8, LocalDateTime.of(2026, 5, 2, 9, 10), StatutDemande.CLOTUREE,
                        PrioriteDemande.BASSE, "legal_0002.pdf");

                tx.commit();
                System.out.println(">>> [DemandeListeServlet] Seeding du registre terminé avec succès.");
            } catch (Exception e) {
                if (tx.isActive())
                    tx.rollback();
                e.printStackTrace();
