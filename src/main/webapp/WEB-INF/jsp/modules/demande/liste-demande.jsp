...
<div class="page-header">
    ...
    <div class="page-header-actions">
        <button id="btn-exporter-demandes" class="btn btn-ghost btn-sm">
            <i class="ti ti-download" aria-hidden="true"></i> Exporter le registre
        </button>
        ...
        <%-- 💡 ALIGNEMENT : Routage unifié vers le contrôleur pour initialiser un contexte de création propre --%>
        <a href="${pageContext.request.contextPath}/demande/liste?mode=create" class="btn btn-primary btn-sm">
            <i class="ti ti-plus" aria-hidden="true"></i> Nouvelle demande
        </a>
    </div>
</div>
...
<div class="card mb-4" style="padding:var(--space-4)">
    <div style="display:flex;gap:var(--space-3);flex-wrap:wrap;align-items:flex-end">
        ...
        <%-- 💡 DYNAMISATION DE LA PRIORITÉ : Options injectées par JS --%>
        <div style="min-width:160px">
...
<%-- Tableau de données géré par dynamique tableau.js --%>
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const demandesData = ${empty demandesJson ? '[]' : demandesJson};
        
        // 💡 1.B PEUPLEMENT DYNAMIQUE DES FILTRES DEPUIS LES DONNÉES DU TABLEAU
        
        // Extraction et injection des STATUTS réels
        const filterStatutSelect = document.getElementById('filterStatutDemande');
        const statutsUniques = [...new Set(demandesData.map(d => d.statutLabel).filter(Boolean))];
        ...
        // Extraction et injection des PRIORITÉS réelles
        const filterPrioriteSelect = document.getElementById('filterPrioriteDemande');
        const prioritesUniques = [...new Set(demandesData.map(d => d.prioriteLabel).filter(Boolean))];
        prioritesUniques.sort().forEach(priorite => {
            const option = document.createElement('option');
            option.value = priorite;
            option.textContent = priorite;
            filterPrioriteSelect.appendChild(option);
        });
        ...
        const demandeTable = new DynamicTable({
            ...
            <td>\${d.requérantNomComplet || 'N/A'}</td>
            <td style="color:var(--text-muted)">\${d.dateSoumissionFormatee || ''}</td>
            <td>
            <span class="badge badge-\${d.prioriteColorClass || 'default'}" style="font-size:10px; font-weight:var(--fw-semibold)">
            \${d.prioriteLabel === 'Urgente' ? '<i class="ti ti-alert-circle" style="font-size:11px" aria-hidden="true"></i> ' : ''}\${d.prioriteLabel || ''}
            </span>
            </td>
            <td>
            <span class="badge badge-\${d.statutColorClass || 'default'}">
            ...
            <%-- 💡 ALIGNEMENT : Mode Lecture Seule (Preview) --%>
            <a href="${pageContext.request.contextPath}/demande/liste?id=\${d.id}&mode=preview" class="btn btn-ghost btn-sm btn-icon" title="Consulter la demande en lecture seule">
            <i class="ti ti-eye" aria-hidden="true"></i>
            </a>
            
            <%-- 💡 ALIGNEMENT : Mode Édition / Traitement --%>
            <a href="${pageContext.request.contextPath}/demande/liste?id=\${d.id}&mode=edit" class="btn btn-ghost btn-sm btn-icon" title="Traiter la demande / Ouvrir le dossier">
        ...
        const mappingColonnesDemandes = {
            numeroSuivi: "Code Suivi",
            typeDemandeLabel: "Type de Demande",
            requérantNomComplet: "Citoyen Requérant",
            dateSoumissionFormatee: "Date de Soumission",
            prioriteLabel: "Priorité",
            statutLabel: "Statut"
        };
        
        document.getElementById('btn-exporter-demandes').addEventListener('click', (e) => {
            e.preventDefault();
            exportCSV(demandesData, mappingColonnesDemandes, 'registre_demandes_administratives');
        });
    });
</script>